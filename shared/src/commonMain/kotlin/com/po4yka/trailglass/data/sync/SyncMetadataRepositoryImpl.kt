...
class SyncMetadataRepositoryImpl(
    ...
    override suspend fun getMetadata(entityId: String, entityType: EntityType): SyncMetadata? = withContext(Dispatchers.IO) {
        logger.trace { "Getting metadata for: $entityType:$entityId" }
        queries.getMetadata(entityId, entityType.name)
            .asFlow()
    ...
    override suspend fun getPendingSync(entityType: EntityType, limit: Int): List<SyncMetadata> = withContext(Dispatchers.IO) {
        logger.trace { "Getting pending sync for: $entityType (limit: $limit)" }
        queries.getPendingSync(entityType.name, limit.toLong())
            .asFlow()
    ...
    override suspend fun getPendingDelete(entityType: EntityType): List<SyncMetadata> = withContext(Dispatchers.IO) {
        logger.trace { "Getting pending delete for: $entityType" }
        queries.getPendingDelete(entityType.name)
            .asFlow()
            .mapToList(Dispatchers.IO)
            .first()
            .map { row ->
    ...
    override suspend fun getAllPendingSync(): List<SyncMetadata> = withContext(Dispatchers.IO) {
        logger.trace { "Getting all pending sync" }
        queries.getAllPendingSync()
            .asFlow()
            .mapToList(Dispatchers.IO)
            .first()
            .map { row ->
    ...
    override suspend fun getPendingSyncCount(): Int = withContext(Dispatchers.IO) {
        ...
        queries.getPendingSyncCount()
            .executeAsOne()
            .toInt()
    }

    override suspend fun getLastSyncedMetadata(): SyncMetadata? = withContext(Dispatchers.IO) {
        logger.trace { "Getting last synced metadata" }
        queries.getLastSyncedMetadata()
            .asFlow()
            .mapToOneOrNull(Dispatchers.IO)
            .first()
            ?.let { row ->
