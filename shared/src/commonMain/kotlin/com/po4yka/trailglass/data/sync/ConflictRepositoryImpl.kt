...
class ConflictRepositoryImpl(
    ...
    override suspend fun getConflict(conflictId: String): StoredConflict? = withContext(Dispatchers.IO) {
        logger.trace { "Getting conflict: $conflictId" }
        queries.getConflict(conflictId)
            .asFlow()
    ...
    override suspend fun getPendingConflicts(): List<StoredConflict> = withContext(Dispatchers.IO) {
        logger.trace { "Getting pending conflicts" }
        queries.getPendingConflicts()
            .asFlow()
    ...
    override suspend fun getConflictCount(): Int = withContext(Dispatchers.IO) {
        logger.trace { "Getting conflict count" }
        queries.getConflictCount()
            .executeAsOne()
            .toInt()
    }
