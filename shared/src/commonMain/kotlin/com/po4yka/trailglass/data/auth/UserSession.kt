 ...
 *
 * This interface provides access to the authenticated user's information. Supports three modes:
 * - Authenticated user (with account)
 * - Guest user (no account, local-only data)
 * - No user (not authenticated, not guest)
 */
interface UserSession {
    /** Returns the current user's ID. Returns null if no user is authenticated. */
    ...
    companion object {
        /** Special user ID for guest mode. When a user continues without an account, this ID is used. */
 ...
 *
 * Currently provides a default user ID for single-user scenarios. This can be extended or replaced with a proper
 * authentication system that integrates with OAuth, Firebase Auth, or custom auth backends.
 */
class DefaultUserSession(
    ...
    companion object {
        /**
         * Default user ID for single-user scenarios. This ensures existing functionality works while allowing future
         * multi-user support.
         */
        const val DEFAULT_USER_ID = "default_user"

        /** Singleton instance for convenience. In production, this should be injected via DI (Koin, etc.). */
        ...
        /** Sets a custom UserSession implementation. Useful for testing or custom auth integrations. */
