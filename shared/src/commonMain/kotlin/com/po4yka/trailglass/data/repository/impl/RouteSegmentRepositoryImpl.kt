...
class RouteSegmentRepositoryImpl(
    ...
    override suspend fun insertRouteSegment(segment: RouteSegment) =
    ...
    override suspend fun getRouteSegmentById(segmentId: String): RouteSegment? =
        withContext(Dispatchers.IO) {
            logger.trace { "Getting route segment by ID: $segmentId" }
            try {
                val segmentRow =
                    queries
                        .getRouteSegmentById(segmentId)
                        .asFlow()
                ...
                val sampleIds =
                    queries
                        .getRouteSegmentSamples(segmentId)
                        .executeAsList()
                        .map { it.id }
                ...
                val pathPoints =
                    queries
                        .getSimplifiedPathPoints(segmentId)
                        .executeAsList()
                        .map { Coordinate(it.latitude, it.longitude) }

                logger.trace {
                    "Found route segment $segmentId with ${sampleIds.size} samples and ${pathPoints.size} path points"
                }
            ...
            try {
                ...
                val segments =
                    segmentRows.map { row ->
                        val sampleIds =
                            queries
                                .getRouteSegmentSamples(row.id)
                                .executeAsList()
                                .map { it.id }

                        val pathPoints =
                            queries
                                .getSimplifiedPathPoints(row.id)
                                .executeAsList()
                                .map { Coordinate(it.latitude, it.longitude) }
    ...
    override suspend fun deleteRouteSegment(segmentId: String) =
        withContext(Dispatchers.IO) {
    ...
    override suspend fun getRouteSegmentCount(): Long =
        withContext(Dispatchers.IO) {
