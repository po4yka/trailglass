...
sealed class PermissionState {
    ...
    /**
     * Permission has been permanently denied (user selected "Don't ask again" on Android, or denied multiple times on
     * iOS). The user must manually enable it in system settings.
     */
...
enum class PermissionType {
    /** Fine location permission (GPS). Required for: Location tracking, map follow mode, place visit detection. */
    LOCATION_FINE,

    /** Coarse location permission (network-based). Required for: Approximate location tracking. */
    LOCATION_COARSE,

    /** Background location permission. Required for: Tracking location when app is in background. */
    LOCATION_BACKGROUND,

    /** Camera permission. Required for: Taking photos for place visits. */
    CAMERA,

    /** Photo library permission. Required for: Accessing and attaching existing photos. */
    PHOTO_LIBRARY,

    /** Notification permission. Required for: Sync notifications, tracking reminders. */
    NOTIFICATIONS
}
